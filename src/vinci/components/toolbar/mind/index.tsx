...
export const MindToolbar = () => {
  const dragImageRef = useRef<HTMLDivElement>(null);

  const onDragStart = (event: DragEvent<HTMLDivElement>) => {
    event.dataTransfer?.setData("vince/drop-to-add-mind", "mind");
  ...
  return (
    <div className={styles.wrap} draggable onDragStart={onDragStart}>
      <Tooltip content="思维导图" position={"rightTop"}>
        <Button type="tertiary" icon={<IconMind />} />
      </Tooltip>

      <div className={styles.dragImage} ref={dragImageRef}>
        <Button type="tertiary" size="small" icon={<IconMind />} />
      </div>
    </div>
  );
};
